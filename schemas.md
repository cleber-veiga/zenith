...
Notes:
- Managers can create/update/delete only the workspaces and projects they created (super users can manage everything).
- Executors/viewers can read only the workspaces/projects they are members of.
- To enforce “viewer sees only some fields”, query `workspaces_viewer` and `projects_viewer` for viewer users in the app.
